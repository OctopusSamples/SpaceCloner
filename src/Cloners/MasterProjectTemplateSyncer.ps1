function Sync-OctopusMasterOctopusProjectWithChildProjects
{
    ...
    if ([string]::IsNullOrWhiteSpace($CloneScriptOptions.TemplateProjectName) -eq $true -or [string]::IsNullOrWhiteSpace($CloneScriptOptions.ChildProjectsToSync) -eq $true)
    {
    ...
    $filteredSourceList = Get-OctopusFilteredList -itemList $sourceData.ProjectList -itemType "Projects" -filters $cloneScriptOptions.TemplateProjectName

    if ($filteredSourceList.Length -ne 1)
    {
        Throw "The project you specified as the template $($CloneScriptOptions.TemplateProjectName) resulted in $($filteredList.Length) item(s) found in the source.  This count must be exactly equal to 1.  Please update the filter."
    }
